...
def disturbance_fn(data: SimData) -> SimData:
    ...
    torque = states.torques[..., 0, :]
    disturbance_torque = jax.random.normal(subkey, torque.shape) * 5e-3
    states = states.replace(torques=states.torques.at[..., 0, :].set(torque + disturbance_torque))
