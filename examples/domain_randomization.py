...
def example_1(sim):
    ...
    batched_sys = domain_randomizer.randomize_friction_from_distribution(
        sim._mjx_model, "uniform", friction_range
    )
...
def example_2(sim):
    ...
    def helper(key):
        return jax.random.uniform(
            key, shape=(1,), minval=friction_range[0], maxval=friction_range[1]
        )
...
def example_5(sim):
    ...
    domain_randomizer = DomainRandomizer(env_num=env_num, seed=0)
    batched_sys = domain_randomizer.randomize_body_positions_from_distribution(
        sim._mjx_model, "uniform", position_range
    )
...
def example_6(sim):
    ...
    def helper(key):
        return jax.random.uniform(
            key, shape=(3,), minval=position_range[0], maxval=position_range[1]
        )
